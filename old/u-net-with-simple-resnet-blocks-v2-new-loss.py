...
aug_pipeline = iaa.Sequential([
            ...
            iaa.OneOf([
                ...
                iaa.Noop(),
                iaa.Affine(rotate=(-10, 10), translate_percent={"x": (-0.25, 0.25)}, mode='reflect', shear=(-12, 12), cval=(0)),
                iaa.Noop(),
                iaa.Affine(shear=(-12, 12), mode='reflect', cval=(0)),
                iaa.Noop(),
...
def generator(features, labels, batch_size, aug_pipeline):
...
model_checkpoint = ModelCheckpoint('../models/' + save_model_name,monitor='my_iou_metric', 
                                   mode = 'max', save_best_only=True, verbose=1)
reduce_lr = ReduceLROnPlateau(monitor='my_iou_metric', mode = 'max',factor=0.5, patience=5, min_lr=0.0001, verbose=1)
tb = TensorBoard(log_dir="../tb_logs/" + basic_name, batch_size=batch_size)



steps_per_epoch = x_train.shape[0] / batch_size

history = model1.fit_generator(generator(x_train, y_train, batch_size, aug_pipeline),
...
if continue_from_epoch > 0:
    model = load_model('../models/finetune-2-' + save_model_name,custom_objects={'my_iou_metric_2': my_iou_metric_2,
                                                   'lovasz_loss': lovasz_loss})
else:
    model1 = load_model('../models/' + save_model_name,custom_objects={'my_iou_metric': my_iou_metric})
...
model_checkpoint = ModelCheckpoint('../models/finetune-2-'+save_model_name,monitor='val_my_iou_metric_2', 
                                   mode = 'max', save_best_only=True, verbose=1)
reduce_lr = ReduceLROnPlateau(monitor='val_my_iou_metric_2', mode = 'max',factor=0.5, patience=5, min_lr=0.0001, verbose=1)
tb = TensorBoard(log_dir="../tb_logs/finetune-2-" + basic_name, batch_size=batch_size)
